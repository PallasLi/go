...
func testTemplate(){
	...
	t2 := template.New("main.tmpl")
	t2 = template.Must(t2.ParseGlob("templates/*.tmpl"))
	err2 := t2.Execute(os.Stdout, nil)
	if err2 != nil {
		log.Fatalf("template execution: %s", err2)
	}


 
	//定义:模板就是将一组文本嵌入另一组文本里
	//传入string--最简单的替换
	...
	//这句tmpl, err := template.New("test").Parse("hello, {{.}}")
	//等于下面的两句muban := "hello, {{.}}" ; tmpl, err := template.New("test").Parse(muban)
	//之后的例子都用两句的方式表达
